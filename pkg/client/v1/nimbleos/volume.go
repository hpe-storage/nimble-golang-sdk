...
// EncodeVolume - Transform Volume to volume type
func EncodeVolume(request interface{}) (*volume, error) {
	reqVolume := request.(*Volume)
	byte, err := json.Marshal(reqVolume)
	if err != nil {
		return nil, err
	}
	respVolumePtr := &volume{}
	err = json.Unmarshal(byte, respVolumePtr)
	return respVolumePtr, err
}

// DecodeVolume Transform volume to Volume type
func DecodeVolume(request interface{}) (*Volume, error) {
	reqVolume := request.(*volume)
	byte, err := json.Marshal(reqVolume)
	if err != nil {
		return nil, err
	...
	respVolume := &Volume{}
	err = json.Unmarshal(byte, respVolume)

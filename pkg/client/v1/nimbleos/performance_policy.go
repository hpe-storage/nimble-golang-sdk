...
// EncodePerformancePolicy - Transform PerformancePolicy to performancePolicy type
func EncodePerformancePolicy(request interface{}) (*performancePolicy, error) {
	reqPerformancePolicy := request.(*PerformancePolicy)
	byte, err := json.Marshal(reqPerformancePolicy)
	if err != nil {
		return nil, err
	}
	respPerformancePolicyPtr := &performancePolicy{}
	err = json.Unmarshal(byte, respPerformancePolicyPtr)
	return respPerformancePolicyPtr, err
}

// DecodePerformancePolicy Transform performancePolicy to PerformancePolicy type
func DecodePerformancePolicy(request interface{}) (*PerformancePolicy, error) {
	reqPerformancePolicy := request.(*performancePolicy)
	byte, err := json.Marshal(reqPerformancePolicy)
	if err != nil {
		return nil, err
	}
	respPerformancePolicy := &PerformancePolicy{}
	err = json.Unmarshal(byte, respPerformancePolicy)
	return respPerformancePolicy, err
}

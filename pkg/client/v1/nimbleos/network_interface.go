...
// EncodeNetworkInterface - Transform NetworkInterface to networkInterface type
func EncodeNetworkInterface(request interface{}) (*networkInterface, error) {
	reqNetworkInterface := request.(*NetworkInterface)
	byte, err := json.Marshal(reqNetworkInterface)
	if err != nil {
		return nil, err
	}
	respNetworkInterfacePtr := &networkInterface{}
	err = json.Unmarshal(byte, respNetworkInterfacePtr)
	return respNetworkInterfacePtr, err
}

// DecodeNetworkInterface Transform networkInterface to NetworkInterface type
func DecodeNetworkInterface(request interface{}) (*NetworkInterface, error) {
	reqNetworkInterface := request.(*networkInterface)
	byte, err := json.Marshal(reqNetworkInterface)
	if err != nil {
		return nil, err
	}
	respNetworkInterface := &NetworkInterface{}
	err = json.Unmarshal(byte, respNetworkInterface)
	return respNetworkInterface, err
}

...
// EncodeShelf - Transform Shelf to shelf type
func EncodeShelf(request interface{}) (*shelf, error) {
	reqShelf := request.(*Shelf)
	byte, err := json.Marshal(reqShelf)
	if err != nil {
		return nil, err
	}
	respShelfPtr := &shelf{}
	err = json.Unmarshal(byte, respShelfPtr)
	return respShelfPtr, err
}

// DecodeShelf Transform shelf to Shelf type
func DecodeShelf(request interface{}) (*Shelf, error) {
	reqShelf := request.(*shelf)
	byte, err := json.Marshal(reqShelf)
	if err != nil {
		return nil, err
	...
	respShelf := &Shelf{}
	err = json.Unmarshal(byte, respShelf)
	return respShelf, err
}

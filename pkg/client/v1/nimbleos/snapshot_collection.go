...
// EncodeSnapshotCollection - Transform SnapshotCollection to snapshotCollection type
func EncodeSnapshotCollection(request interface{}) (*snapshotCollection, error) {
	reqSnapshotCollection := request.(*SnapshotCollection)
	byte, err := json.Marshal(reqSnapshotCollection)
	if err != nil {
		return nil, err
	}
	respSnapshotCollectionPtr := &snapshotCollection{}
	err = json.Unmarshal(byte, respSnapshotCollectionPtr)
	return respSnapshotCollectionPtr, err
}

// DecodeSnapshotCollection Transform snapshotCollection to SnapshotCollection type
func DecodeSnapshotCollection(request interface{}) (*SnapshotCollection, error) {
	reqSnapshotCollection := request.(*snapshotCollection)
	byte, err := json.Marshal(reqSnapshotCollection)
	if err != nil {
		return nil, err
	}
	respSnapshotCollection := &SnapshotCollection{}
	err = json.Unmarshal(byte, respSnapshotCollection)
	return respSnapshotCollection, err
}

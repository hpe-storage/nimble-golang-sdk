...
// EncodeVolumeCollection - Transform VolumeCollection to volumeCollection type
func EncodeVolumeCollection(request interface{}) (*volumeCollection, error) {
	reqVolumeCollection := request.(*VolumeCollection)
	byte, err := json.Marshal(reqVolumeCollection)
	if err != nil {
		return nil, err
	}
	respVolumeCollectionPtr := &volumeCollection{}
	err = json.Unmarshal(byte, respVolumeCollectionPtr)
	return respVolumeCollectionPtr, err
}

// DecodeVolumeCollection Transform volumeCollection to VolumeCollection type
func DecodeVolumeCollection(request interface{}) (*VolumeCollection, error) {
	reqVolumeCollection := request.(*volumeCollection)
	byte, err := json.Marshal(reqVolumeCollection)
	if err != nil {
		return nil, err
	}
	respVolumeCollection := &VolumeCollection{}
	err = json.Unmarshal(byte, respVolumeCollection)
	return respVolumeCollection, err
}

...
// EncodeDisk - Transform Disk to disk type
func EncodeDisk(request interface{}) (*disk, error) {
	reqDisk := request.(*Disk)
	byte, err := json.Marshal(reqDisk)
	if err != nil {
		return nil, err
	}
	respDiskPtr := &disk{}
	err = json.Unmarshal(byte, respDiskPtr)
	return respDiskPtr, err
}

// DecodeDisk Transform disk to Disk type
func DecodeDisk(request interface{}) (*Disk, error) {
	reqDisk := request.(*disk)
	byte, err := json.Marshal(reqDisk)
	if err != nil {
		return nil, err
	...
	respDisk := &Disk{}
	err = json.Unmarshal(byte, respDisk)
	return respDisk, err
}

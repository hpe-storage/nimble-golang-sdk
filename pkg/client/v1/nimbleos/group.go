...
// EncodeGroup - Transform Group to group type
func EncodeGroup(request interface{}) (*group, error) {
	reqGroup := request.(*Group)
	byte, err := json.Marshal(reqGroup)
	if err != nil {
		return nil, err
	}
	respGroupPtr := &group{}
	err = json.Unmarshal(byte, respGroupPtr)
	return respGroupPtr, err
}

// DecodeGroup Transform group to Group type
func DecodeGroup(request interface{}) (*Group, error) {
	reqGroup := request.(*group)
	byte, err := json.Marshal(reqGroup)
	if err != nil {
		return nil, err
	...
	respGroup := &Group{}
	err = json.Unmarshal(byte, respGroup)
	return respGroup, err
}

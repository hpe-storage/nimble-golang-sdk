...
func NewNsGroupService(ip, username, password, apiVersion string, synchronous, tenantAware bool) (gs *NsGroupService, err error) {
 if apiVersion != "v1" {
  return nil,fmt.Errorf("NewNsGroupService: unsupported %s sdk API version",apiVersion)
 }
 client, err := client.NewClient(ip, username, password, apiVersion, synchronous, tenantAware)
 if err != nil {
  return nil, err
 }
